...
$(document).ready(function(e) {
	...
	showItineraryDetail=function(arguiid){
		...
		global_showloadinginit=true;
		var _url = "data_ajax/myitinerary_content3.cfm?";
	...
	lighting = function () {
		$("#buttonMyItinerary").animate({opacity:'+=10'}, 500);
    	$("#buttonMyItinerary").animate({opacity:'-=0.5'}, 500, lighting);
	}
...
//	});
	...
	$(document).on("click", "#buttonSaveAndFile", function(e){
		...
		var _divdayboxcontent =$("#id_plan");
		/*var _ui_idObj = _divdayboxcontent.find("#ui_id")*/;
		var _ui_id = document.getElementById("ui_id").value;
		/*var _ui_id = _ui_idObj.val();*/
		/*alert(_ui_id);*/
		if(cf_sid==""){
			pedir_sesion(1,"copyItinerary("+_ui_id+")");
			$("#buttonMyItinerary").css("display", "inline-block");
			/*alert("refresh");*/
			return;
		}
	...
	$(document).on("click",".shareit #buttonTipItinerary",function(e){
		...
		$('body').append('<div id="mask" class="mask"></div>');
		$('#mask').fadeIn(300);
		$('#tipitinerary_description').attr('readonly', '')
	});  
	...
	$(document).on("click","#buttonMapItinerary2",function(e){
		e.preventDefault();
		openMapItineraryPopup($(this).data("id"));
	});   
