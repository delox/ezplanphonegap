function changebegindate(date){
	...
	var urlconfig = {action:"updatebegindate",datef:datef,datet:datet,datenew:newdate,type:hayfechas,useddays:useddays};
...
function addday(){
	...
	if(!datef || datef==''){
		...
		if(!numdaystoadd || numdaystoadd<1 || isNaN(numdaystoadd)){alert('Please Type a valid number of days');return false;}
...
function delday(){
	if(useddays==1){alert("You can't eliminate last day");return false;}
	var datef=$.cookie("EZDATE1");
	if(!datef || datef==''){datef='';}
	...
	if(esrango==1){
		...
		for(x=toloop[0];x<=toloop[1];x++){
			...
			if(loads && loads!=''){var tienealgo=true;daytienen.push(x);}
		...
		if(loads && loads!=''){var tienealgo=true;}
		...
		if(tienealgo){
			...
			var url = "https://myezplan.com/mobile/appdata/data_ajax/mapcity_content3.cfm?rnu=5131616516651";
			var urlconfig = {action:"delfullday",date1:datef,useddays:useddays,num:daytodelete,esrango:esrango};
			$("#funcsparaadddates").load(url,urlconfig,function (response, status, xhr){
					switch (status){
						case "success":
							$('.sortable.grid li').on('click', function(e){
...
$('#addadate').hide();
	...
	var date2=$.cookie('EZDATE2');
			var url = "https://myezplan.com/mobile/appdata/data_ajax/mapcity_content3.cfm?rnu=5131616516651";
			var urlconfig = {action:"adddatestofull",date1:date1,date2:date2,datenew:newdate,useddays:useddays};
			$("#funcsparaadddates").load(url,urlconfig,function (response, status, xhr){
					switch (status){
						case "success":
							$('.sortable.grid li').on('click', function(e){
