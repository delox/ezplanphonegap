...
function Open_reportaproblem_p(plid){
		...
		var popZindez = (parseInt($("#roadmapplace-box").css("z-index"))) + 5;
		$('#mask-reportview').css({'z-index' : popZindez});		
		$('#mask-reportview').fadeIn(300);
		var _url = "data_ajax/reportaproblem_content.cfm";
		var _urlconfig = {action:'reportaproblem_p',plid:plid,user:cf_sid};
...
function ideasrouteclick(index){
	if (faqpage != "mapcity"){
		var id=arrayideasr[index].id
		var yatiene=1;
		if(arrayideasr[index].desc_==''){var yatiene=0;}
...
var texto='<table style="border-collapse:collapse;">'+texto+'</table>';
...
function textodepersonales(){
	...
	for(pm=0;pm<personalmrks.length;pm++){
		var texto=texto+'<tr class="personalmrk_'+(pm)+'" onmouseover="mrkpersonalover('+(pm+1)+');" onmouseout="mrkpersonalout('+(pm+1)+');" onclick="mrkpersonalclick('+(pm+1)+');"><td style="width:40px;text-align:center;"><img src="'+personalmrks[pm].getIcon()+'" height="35px;"></td><td>'+personalmrks[pm].getTitle()+'</td></tr>';
	}
	var texto='<div style="max-height:350px; overflow:auto; padding: 5px 5px 5px 5px;max-width: 670px;width:95%;border: solid thin #ddd;margin:5px;"><table style="border-collapse:collapse;">'+texto+'</table></div>';
...
function mrkpersonalclick(oindex){
	var index=oindex-1;
	beforezoom=mapcitypopup.mapconfig.google.map.getZoom();
	beforecenter=mapcitypopup.mapconfig.google.map.getCenter();
	personalmrks[index].userinfowindow.open(mapcitypopup.mapconfig.google.map,personalmrks[index]);	
	var ppcenter = personalmrks[index].map.center;
	var pplat = personalmrks[index].map.center.A;
	var pplng = personalmrks[index].map.center.F;
	var ppname = personalmrks[index].title; 
	mapcitypopup.mapconfig.google.map.setCenter(ppcenter);
	mapcitypopup.mapconfig.google.map.setZoom(15);
	$('#body_map_canvas .gm-style').removeClass('gmstylefull');
	var pplace = '<div style="padding: 5px 5px 5px 5px;max-width: 670px;width:95%;" class="box-gray-c"><table><tbody><tr><td><img height="35" src="ezmapas/maps_pics/basic/userplace.png"></td><td><span style="font-weight:bold;color:#c00000;">'+ppname+'</span></td></tr></tbody></table><span><a id="buttdel_ir_2" value="Eliminate" onclick="deletepersonal_ya('+"'"+ppname+"'"+','+oindex+');" style=""><img src="https://lh4.googleusercontent.com/-J8QLIopkdW4/VOdmvVtfapI/AAAAAAANhJA/FcImfqWSudY/s46/basura3.png" height="35">&nbsp;</a></span><span><a value="Edit" onclick="opencalforpersonals('+"'"+ppname+"'"+','+oindex+');"><img src="https://lh3.googleusercontent.com/-FfGsgqD06X4/VEm0BLONY7I/AAAAAAANg7k/2JnRmAw-7_s/s55/183.png" height="35"></a></span>&nbsp;<span><a href="javascript:;" class="" style="" onclick="sugestplace('+"'"+ppname+"'"+','+"'("+pplat+","+pplng+")'"+','+window.citid+');" title="" alt="Close Window alt"><img height="35px" src="https://lh6.googleusercontent.com/-m5bw1hqOADw/VNQWdgi3lpI/AAAAAAANhG8/CjBIzmgVm3g/s45/Suggestions%2520Active.png"></a></span>&nbsp;<span><a href="javascript:;" style="font-weight:bold;" onclick="returnfrompp('+index+');"><img src="https://lh5.googleusercontent.com/-Cb5dhSGmMSA/U5I5STA1muI/AAAAAAANfGk/1rOXlx8756Q/s52/Untitled8.png" style="" height="35px"></a></span></div>';
	closesubroutedata();
	$("#transport_descs").append(pplace);
}
...
if(typeof(global_showbutton)!='undefined'){if(global_showbutton){global_showbutton=false;mapcitypopup.showSelPlaces();};}
	mapcitypopup.mapconfig.google.map.setCenter(beforecenter);
	mapcitypopup.mapconfig.google.map.setZoom(beforezoom);
	closesubroutedata();	
	c_pp=false;
	create_pp();
}
...
function returnfrompp(index){
	personalmrks[index].userinfowindow.close(mapcitypopup.mapconfig.google.map,personalmrks[index]);
	mapcitypopup.mapconfig.google.map.setCenter(beforecenter);
	mapcitypopup.mapconfig.google.map.setZoom(beforezoom);
