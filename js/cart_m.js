...
function markdealasbuyed(ruta_id, cityname, plan_name, ruta_name, Title, BuyNowLink, coordenadas, serviceprovider_id, buyed){
	...
	var todo=0;
	if(che){var todo=1;}
		var _url = "https://myezplan.com/mobile/appdata/data_ajax/cart_ajax_content.cfm?"+randomNumberUrl()+"";
...
if(lola && lola !=''){
	...
	var lola=sessionStorage.cf_user.replace(/\s+/g, '');
	document.getElementById(idunico+'_'+opid+'_email').value=lola;
}
togenerataauto=[];
	...
	for(c=0;c<cook_.length;c++){
		...
		var content=content+'<tr><td align="center"><input style="width:100%" class="select_m" onfocus="agetochange='+"'"+x+'age_'+catid+'_'+idunico+'_'+opid+"';"+'nametochange='+"'"+x+'name_'+catid+'_'+idunico+'_'+opid+"';"+'" onblur="updateoptdata('+catid+','+"'"+idunico+"'"+','+x+','+"'"+opid+"'"+');" type="text" id="'+x+'name_'+catid+'_'+idunico+'_'+opid+'" value="'+name+'" ></td><td align="center">&nbsp;</td><td><input class="select_m" size="4" onblur="updateoptdata('+catid+','+"'"+idunico+"'"+','+x+','+"'"+opid+"'"+');" type="text" id="'+x+'age_'+catid+'_'+idunico+'_'+opid+'" value="'+age+'"></td><td>&nbsp;</td><td align="center"><input type="radio" name="lolo"></td></tr>';
togenerataauto.push("#"+x+'name_'+catid+'_'+idunico+'_'+opid+'!'+'#'+x+'age_'+catid+'_'+idunico+'_'+opid)
	...
	for(c=0;c<cook_.length;c++){
		...
		var content=content+'<tr><td align="center"><input style="width:100%" onfocus="agetochange='+"'"+x+'age_'+catid+'_'+idunico+'_'+opid+"';"+'nametochange='+"'"+x+'name_'+catid+'_'+idunico+'_'+opid+"';"+'" onblur="updateoptdata('+catid+','+"'"+idunico+"'"+','+x+','+"'"+opid+"'"+');" type="text" id="'+x+'name_'+catid+'_'+idunico+'_'+opid+'" value="'+name+'" ></td><td align="center">&nbsp;</td><td><input class="select_m" size="4" onblur="updateoptdata('+catid+','+"'"+idunico+"'"+','+x+','+"'"+opid+"'"+');" type="text" id="'+x+'age_'+catid+'_'+idunico+'_'+opid+'" value="'+age+'"></td><td>&nbsp;</td><td align="center"><input type="radio" name="lolo"></td></tr>';
togenerataauto.push("#"+x+'name_'+catid+'_'+idunico+'_'+opid+'!'+'#'+x+'age_'+catid+'_'+idunico+'_'+opid)
		}
	}	
	...
	for(c=0;c<cook_.length;c++){
		...
		var content=content+'<tr><td align="center"><input style="width:100%" class="select_m" class="select_m" onfocus="agetochange='+"'"+x+'age_'+catid+'_'+idunico+'_'+opid+"';"+'nametochange='+"'"+x+'name_'+catid+'_'+idunico+'_'+opid+"';"+'" onblur="updateoptdata('+catid+','+"'"+idunico+"'"+','+x+','+"'"+opid+"'"+');" type="text" id="'+x+'name_'+catid+'_'+idunico+'_'+opid+'" value="'+name+'" ></td><td align="center">&nbsp;</td><td><input class="select_m" size="4" onblur="updateoptdata('+catid+','+"'"+idunico+"'"+','+x+','+"'"+opid+"'"+');" type="text" id="'+x+'age_'+catid+'_'+idunico+'_'+opid+'" value="'+age+'"></td><td>&nbsp;</td><td align="center"><input type="radio" name="lolo"></td></tr>';
togenerataauto.push("#"+x+'name_'+catid+'_'+idunico+'_'+opid+'!'+'#'+x+'age_'+catid+'_'+idunico+'_'+opid)
		}
	}	
...
armaautos();
	...
	getcartpurchasedetails=function(){
		...
		/*global_showloadinginit=true;*/
		var _url = "https://myezplan.com/mobile/appdata/data_ajax/cart_ajax_content.cfm?"+randomNumberUrl()+"";
		var _urlconfig = {action: "getcartpurchasedetails",upuserid:sessionStorage.cf_sid,cartinfo:sessionStorage.cart,externalcart:sessionStorage.externalcart,cartinfo2:sessionStorage.cart2,carttotalscoup:sessionStorage.carttotals_coupons};
					...
					$(".date-inline-c").datepicker({
						...
						*/
						, onSelect: function(dateText, datePicker) {
							...
							var _ocode 	= 	$(this).attr('data-ocode');
							var _ruta 	= 	$(this).attr('data-ruta');
							_input.attr('value', dateText);
							cookiedates(_ocode,_ruta);
							$(this).hide();
						}
					});
					var _cuoparray = JSON.parse(sessionStorage.cartcoupons);
					var _coupcode = _cuoparray[0].couponcode;
					//alert(_coupcode);
					$("#noapplcoup"+_coupcode).hide();
					$("#applcoup"+_coupcode).show();
					break;
				default:
					break;
			}
	...
	getcartmissingdeals=function(){
		...
		var addedarray=[];
		for(x=1;x<=numdays;x++){
			var loads=$.cookie("EZMAPCITYPLACES_day"+x).split(',');
			for(l=0;l<loads.length;l++){addedarray.push(loads[x]);}
		}
		var _url = "https://myezplan.com/mobile/appdata/data_ajax/cart_ajax_content.cfm?"+randomNumberUrl()+"";
		var _urlconfig = {action: "cartmissingdeals",added:loads,upuserid:sessionStorage.cf_sid,cartinfo:sessionStorage.cart,externalcart:sessionStorage.externalcart,cartinfo2:sessionStorage.cart2};
	...
	openMyCoupons_ajax=function(){
		...
		var _url = "https://myezplan.com/mobile/appdata/data_ajax/cart_ajax_content.cfm";
	...
	validatecoupon=function(arg_showmessage,arg_applycoupon){
		...
		var _data = {action:"validatecoupon",coupon:_val,upuserid:sessionStorage.cf_sid};
		$.ajax({
			type: "POST",
			dataType:"json",
			...
			success: function(response){
				if($.trim(response.MESSAGE).length>0 && _showmessage){
					//if (response.ACTION == "applied"){
						//var action = confirm(response.MESSAGE+" Woudl you like to remove it from this ...?");
						alert(response.MESSAGE);
						/*if(action == true){
							$.post("https://myezplan.com/mobile/appdata/data_ajax/cart_ajax_events.cfm?action=deletecoupqueue&couptodel="+_val+"&upuserid="+sessionStorage.cf_sid);
	...
	applycoupon=function(){
		...
		var _data = {action:"applycoupon",coupon:_val,upuserid:sessionStorage.cf_sid,coupArr:sessionStorage.cartcoupons,totalcoupons:sessionStorage.carttotals_coupons};
		...
		$.ajax({
			...
			success: function(response){
				//alert(response);				
				var couponvars = response.split("*");
				window.coupresponse =	JSON.parse(couponvars[0]);
				sessionStorage.cartcoupons = couponvars[1];				
				sessionStorage.carttotals_coupons = couponvars[2];
				getcartpurchasedetails();
				getcartmissingdeals();
				/*if(response.OK){
					getcartpurchasedetails();
	...
	updatecartitems=function(){
		...
		var _data = _formserialized+"&action=updatecartitems&upuserid="+sessionStorage.cf_sid;
		$.ajax({
			type: "POST",
			url: "https://myezplan.com/mobile/appdata/data_ajax/cart_ajax_events.cfm",
			async:true, 
...
$(document).ready(function(e) {
	...
	$(document).on("change",".cartitemnumber",function(e) {
		...
		UpdateArrayItem(window.cartArr, idunico, catid, _obj.val());		
	...
	$(document).off("click","#buttonapplycoupon");
	...
	$(document).on("click",".cart-city-item .close",function(e) {
		...
		var _data = "action=deletecartitem&rid="+_ruta_idunico+"&upuserid="+sessionStorage.cf_sid;
		deleteArrayItem(window.cartArr,_ruta_idunico);
		/*$.ajax({
			type: "POST",
			url: "https://myezplan.com/mobile/appdata/data_ajax/cart_ajax_events.cfm",
			async:false, 
	...
	deleteArrayItem = function (arr,item){
		for(var index in arr){
			if (arr[index].placeid == item) {
				arr.splice(index, 1);			
			}
		}
		...
		if(sessionStorage.cart == "[]"){
			sessionStorage.cart = JSON.stringify([{placeid:"N"}]);
			$("#cart-panel").panel("close");
		}
	}
	UpdateArrayItem = function (arr,item,tcid,qunt){
		var lineToChange = "ticketinfostruct['"+tcid+"'].quantity";
		//alert(lineToChange);
		for(var key in arr){
			if(arr[key].placeid==item){
				console.log(key);
				arr[key].ticketinfostruct[tcid].quantity = qunt; 
			}
		}
		sessionStorage.cart2 =  JSON.stringify(window.cartArr2);
		sessionStorage.cart  =  JSON.stringify(window.cartArr);
	...
	$(".cart-page-m").hide();
	document.getElementById('showmdeals').style.backgroundColor='lightgray';
	ShowHideDiv = function (buttom, div){
		if($("."+div).css('display') == 'none'){
			document.getElementById(buttom).style.backgroundColor="rgb(23, 55, 94)";
			$("."+div).show();
		}else{
			document.getElementById(buttom).style.backgroundColor='lightgray';
			$("."+div).hide();
		}		
	}
	
	$('#faq-cart').click(function(e) {
		e.stopPropagation();e.preventDefault();
		...
		var _url = "https://myezplan.com/mobile/appdata/data_ajax/faq_ajax_content.cfm?page=shoppingcart";
	...
	resetcoupinit=function(){	
		var _data = {action:"resetcart",resetcartaction:"update",resetcartval:"0",upuserid:sessionStorage.cf_sid};
		$.ajax({
			type: "POST",
			//dataType:"json",
			url: "https://myezplan.com/mobile/appdata/data_ajax/cart_ajax_events.cfm",
	...
	resetcoupons=function(_type){
		sessionStorage.carttotals_coupons = 0;
		sessionStorage.cartcoupons = JSON.stringify([]);
		if(_type!="noapply" || typeof _type == 'undefined'){
			$.post("https://myezplan.com/mobile/appdata/data_ajax/cart_ajax_events.cfm?action=resetcart&resetcartaction=clear&upuserid="+sessionStorage.cf_sid);
		}
		getcartpurchasedetails();		
	}
	noapplycoupon=function(coupid){
		$.post("https://myezplan.com/mobile/appdata/data_ajax/cart_ajax_events.cfm?action=deletecoupqueue&couptodel="+coupid+"&upuserid="+sessionStorage.cf_sid);
		resetcoupons('noapply');
	}
	var cartpanel = document.getElementById("cart-panel");
	checkresetcart=function(){		
		var _data = {action:"checkresetcart",upuserid:sessionStorage.cf_sid};
		$.ajax({
			type: "POST",
			//dataType:"json",
			url: "https://myezplan.com/mobile/appdata/data_ajax/cart_ajax_events.cfm",
	...
	if(cartpanel.style.display == "block"){
		var checkcouptime = setInterval(function(){checkresetcart()}, 1000);
	}
	$( "#cart-panel" ).on( "panelbeforeclose", function( event, ui ) {
		clearInterval(checkcouptime);
	});
});

function showplaninfo(planid,plantype){
	$(".load_img").css("display","block");
		var url = "https://myezplan.com/mobile/appdata/data_ajax/cart_ajax_content.cfm?"+randomNumberUrl()+"";
