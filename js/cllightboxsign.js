...
function send_lightbox_signin(){
	...
	if(_usulo=='' || _paslo==''){
		_msg.show();
		showMessageError(_msg,'All information must be complete',300,'error','top');
		hideMessageError(_msg,false,300);			
		if(_usulo==''){
	...
	if(!_isemail){
		_msg.show();
		showMessageError(_msg,'Invalid Email. Please try again',300,'error','top');
		hideMessageError(_msg,false,300);
	...
	$.ajax({
	type: "POST",
	url: "/mobile/appdata/data_ajax/login_ajax.cfm",
	cache:false,
	async:false, 
	...
	success: function(datos)
		{
			if(datos.indexOf('xnox')!=-1)
			{
				...
				_msg.show();
				showMessageError(_msg,'Incorrect Email or Password. Please try again',300,'error','top');
				hideMessageError(_msg,false,300);				
			} 
			else
			{ 
				showMessageError(_msg,'Login successful',300,'success','top');
				hideMessageError(_msg,false,300);
...
function send_lightbox_signup(){
	...
	if(signup_country==''){
		document.getElementById('inc_msgr').style.display='block';
		document.getElementById('inc_msgr').innerHTML='Invalid Country. Please try again';
	...
	$.ajax({
		...
		success: function(datos){
			  if(datos.indexOf("xnox")!=-1)
			  {
					...
					showMessageError(errmsj,'E-mail not found. Please try again.',300,'error','top');
					$("#send_pass").focus().select();
					hideMessageError(errmsj,false,300);
			  }else{
					//document.getElementById('inc_msgf').style.display='block';
					//document.getElementById('inc_msgf').innerHTML='Your password has been sent to<br /> your e-mail account.';
					showMessageError(errmsj,'Your password has been sent to<br /> your e-mail account.',300,'success','top');
					$("#send_pass").focus().select();
					hideMessageError(errmsj,false,300);
