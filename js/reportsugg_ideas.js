			function add_type(){
				var value=document.getElementById('selde_type').value;
				if(value==''){return false;}
				//ver_cambio(value);
				var texto=document.getElementById('detail_type').value;
				var espacio=' ';
				if(texto==''){var espacio='';}
				var texto=texto+espacio+value;
				document.getElementById('detail_type').value=texto;
			}
			function add_hours(){
			...
			function add_difficulty(){
				var value=document.getElementById('selde_difficulty').value;
				if(value==''){return false;}
				var texto=document.getElementById('detail_difficulty').value;

				var espacio=' ';
				if(texto==''){var espacio='';}
				var texto=texto+espacio+value;
				document.getElementById('detail_difficulty').value=texto;
			}
			function add_Distance(){
				var value=document.getElementById('selde_Distance').value;
				if(value==''){return false;}
				var texto=document.getElementById('detail_Distance').value;
				var espacio=' ';
				if(texto==''){var espacio='';}
				var texto=texto+espacio+value;
				document.getElementById('detail_Distance').value=texto;
			}
...
function opentransfaq(){
			 if(rutaostops==0){var pagefaq='report_ideas_rutas';}
			 if(rutaostops==1){var pagefaq='report_ideas_stops';}
...
function selstops(){
$('#control_route').prop("src","https://lh3.googleusercontent.com/-IpnwTDPvKJ8/VEE9ewLerrI/AAAAAAANgqk/oj0a3G6Oes0/s53/route%2520b%2526w%2520small.png");
...
function selroute(){
$('#control_route').prop("src","https://lh3.googleusercontent.com/-uPG50w9EmkM/VEDXQ-LPnnI/AAAAAAANgqE/VvaMNuGpOac/s53/9.png");
...
if(rutaostops==0){
	...
	for(s=0;s<stops.length;s++){		
		...
		stops[s].infowindow=new google.maps.InfoWindow(
					{ content: 'Name<br/><input id="stopname" type="text" value="'+stops[s].name+'"/><br/>Description<br/><input id="stopdesc" type="text" value="'+stops[s].desc+'"/><br/><input type="button" value="Change" onclick="changestopdata('+s+');"/>&nbsp;<input type="button" value="Delete" onclick="delstopdata('+s+');"/>',
...
if(rutaostops==0){
		...
		locationMarker.infowindow=new google.maps.InfoWindow(
					{ content: '<input type="button" value="Delete" onclick="deletemarker('+locationMarker.index+');"/>',
						size: new google.maps.Size(250,250)
					}); 
...
// }
		...
		locationMarker.infowindow=new google.maps.InfoWindow(
					{ content: 'Name<br/><input id="stopname" type="text" value="'+locationMarker.name+'"/><br/>Description<br/><input id="stopdesc" type="text" value="'+locationMarker.desc+'"/><br/><input type="button" value="Change" onclick="changestopdata('+locationMarker.index+');"/>&nbsp;<input type="button" value="Delete" onclick="delstopdata('+locationMarker.index+');"/>',
						size: new google.maps.Size(250,250)});
		google.maps.event.addListener(locationMarker, "click", function() {
			for(s=0;s<stops.length;s++){
...
markers[m].infowindow=new google.maps.InfoWindow(
					{ content: '<input type="button" value="Delete" onclick="deletemarker('+markers[m].index+');"/>',
						size: new google.maps.Size(250,250)
					}); 
}
}
